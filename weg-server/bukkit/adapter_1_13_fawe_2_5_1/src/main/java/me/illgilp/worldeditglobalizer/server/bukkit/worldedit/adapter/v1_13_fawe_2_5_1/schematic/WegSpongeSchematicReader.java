...
public class WegSpongeSchematicReader extends NBTSchematicReader {
    ...
    private static final Logger LOGGER = Logger.getLogger(WegSpongeSchematicReader.class.getCanonicalName());
     ...
     */
    public WegSpongeSchematicReader(com.sk89q.jnbt.NBTInputStream inputStream) {
    ...
    public Clipboard read() throws IOException {
        ...
        if (schematicVersion == 1) {
            ...
            if (dataVersion < 0) {
                LOGGER.warning("Schematic has an unknown data version (" + dataVersion + "). Data may be incompatible.");
                // Do not DFU unknown data
                dataVersion = liveDataVersion;
            }
            if (dataVersion > liveDataVersion) {
                LOGGER.warning("Schematic was made in a newer Minecraft version (" + dataVersion + " > " + liveDataVersion + "). Data may be incompatible.");
            } else if (dataVersion < liveDataVersion) {
                fixer = platform.getDataFixer();
                if (fixer != null) {
                    LOGGER.info("Schematic was made in an older Minecraft version (" + dataVersion + " < " + liveDataVersion + "), will attempt DFU.");
                } else {
                    LOGGER.info("Schematic was made in an older Minecraft version (" + dataVersion + " < " + liveDataVersion + "), but DFU is not available. Data may be incompatible.");
                }
            }
    ...
    private BlockArrayClipboard readVersion1(CompoundTag schematicTag) throws IOException {
        ...
        if (metadataTag != null && metadataTag.containsKey("WEOffsetX")) {
            ...
            Map<String, com.sk89q.jnbt.Tag> metadata = metadataTag.getValue();
        ...
        Map<String, com.sk89q.jnbt.Tag> paletteObject = requireTag(schematic, "Palette", CompoundTag.class).getValue();
        ...
        for (String palettePart : paletteObject.keySet()) {
            ...
            try {
                ...
                LOGGER.warning("Invalid BlockState in palette: " + palettePart + ". Block will be replaced with air.");
        ...
        if (tileEntities != null) {
            List<Map<String, com.sk89q.jnbt.Tag>> tileEntityTags = tileEntities.getValue().stream()
                .map(tag -> (CompoundTag) tag)
                .map(CompoundTag::getValue)
                .collect(Collectors.toList());

            for (Map<String, com.sk89q.jnbt.Tag> tileEntity : tileEntityTags) {
                int[] pos = requireTag(tileEntity, "Pos", IntArrayTag.class).getValue();
                final BlockVector3 pt = BlockVector3.at(pos[0], pos[1], pos[2]);
                Map<String, com.sk89q.jnbt.Tag> values = Maps.newHashMap(tileEntity);
                ...
                values.remove("Pos");
                if (fixer != null) {
                    tileEntity = ((CompoundTag) com.sk89q.jnbt.AdventureNBTConverter.fromAdventure(
                        fixer.fixUp(
                    ...
                    for (CompoundTag ent : entities) {
                        if (ent.containsKey("WEGTypeId")) {
                            ...
                            if (EntityTypes.get(id.toLowerCase()) != null) {
                                ...
                                Map<String, com.sk89q.jnbt.Tag> entM = new HashMap<>(ent.getValue());
                                ...
                                if (ent.containsKey("Pos")) {
                                    ...
                                    if (ent.containsKey("Rotation")) {
                                        List<FloatTag> rot = ent.getList("Rotation", FloatTag.class);
                                        clipboard.createEntity(new Location(clipboard, entX, entY, entZ, rot.get(0).getValue(), rot.get(1).getValue()), new BaseEntity(entityType, new CompoundTag(entM)));
                                    }
                                }
                            }
                        }
    ...
    private void readBiomes(BlockArrayClipboard clipboard, Map<String, com.sk89q.jnbt.Tag> schematic) throws IOException {
        ...
        for (Entry<String, com.sk89q.jnbt.Tag> palettePart : paletteTag.getValue().entrySet()) {
        ...
        while (biomeJ < biomes.length) {
            ...
            for (int y = 0; y < clipboard.getRegion().getHeight(); y++) {
                BlockVector3 v3 = min.add(x, y, z);
                clipboard.setBiome(v3, type);
            }
            biomeIndex++;
        }
    }

    private void readEntities(BlockArrayClipboard clipboard, Map<String, com.sk89q.jnbt.Tag> schematic) throws IOException {
        List<com.sk89q.jnbt.Tag> entList = requireTag(schematic, "Entities", ListTag.class).getValue();
        if (entList.isEmpty()) {
            return;
        }
        for (com.sk89q.jnbt.Tag et : entList) {
            if (!(et instanceof CompoundTag)) {
                continue;
            }
            CompoundTag entityTag = (CompoundTag) et;
            Map<String, com.sk89q.jnbt.Tag> tags = entityTag.getValue();
            String id = requireTag(tags, "Id", StringTag.class).getValue();
            entityTag = entityTag.createBuilder().putString("id", id).remove("Id").build();

            if (fixer != null) {
                entityTag = ((CompoundTag) com.sk89q.jnbt.AdventureNBTConverter.fromAdventure(
                    fixer.fixUp(
            ...
            if (entityType != null) {
                ...
                LOGGER.warning("Unknown entity when pasting schematic: " + id);
            }
        }
    }
